  ...
  $.fn.CodehimClock = function (options) {
    ...
    return this.each(function () {
      ...
      do {
        ...
        $(num).html(r).addClass($thisClass).appendTo(clockDale);
        r++;
      } while (r <= 12);
      ...
      $(clockCenter).addClass("clock-center").appendTo(clockDale);

      $(clockLogo)
        .html("Coded" + "<span>Clock</span>")
        .addClass("clock-logo")
        .appendTo(clockDale);
      ...
      $(dayShow).html(days[$thisDay]).addClass("day").appendTo(dateSticker);

      $(dateShow).html($thisDate).addClass("date").appendTo(dateSticker);

      if (setting.showDate == true) {
        $(dateSticker).addClass("date-box").appendTo(clockDale);
      }

      $(daleinner).addClass("dale-inner").appendTo(clockDale);

      $(spinner1).addClass("logo-outer").appendTo(clockDale);

      $(spinner2).addClass("logo-outer1").appendTo(clockDale);

      $(clockDale).addClass("codehim-clock-dale").appendTo(this);
